...
`net2.0` is fully pushed as of **v5.9** (`0bd9668`) — nothing sitting local-only there right now (check `git log origin/main..HEAD` in a fresh clone to be sure this hasn't drifted since). **v5.9 is built and locally verified but not yet published as a GitHub release** — the checksums in `netportal/README.md` need updating first, then it's a normal release-publish step.

What's genuinely pending: **`netportal-qcow2`, `netportal-vmware`, and `netportal-bare` are stale** — all three were last built against v5.6 content and have not been rebuilt against v5.7, v5.8, or v5.9. Concretely, none of them have Copy Node, Snapshot as Image, the v1.1 branding, the `admin`/`netportal` default credentials, the shared-telnet console fix, or the two `image2` fixes yet. To bring them current: build a `.run` from the latest `net2.0` (`deploy/scripts/build-run.sh`), then run `deploy/scripts/build-qcow2.sh --run <path>` and `deploy/scripts/build-vmware-iso.sh --run <path>`, then publish all three the same way the v5.6 ones were done (see the repo table above).
